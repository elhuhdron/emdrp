...
void ConvNetThread::initLayer(PyObject* paramsDict, int replicaID) {
    ...
    if (type == "fc") {
        ...
        _nameLayerMap[name] = new DropoutLayer(this, paramsDict, replicaID, false);
        ...
        _nameLayerMap[name] = new SumLayer(this, paramsDict, replicaID);
    } else if (type == "whiten") {
        _nameLayerMap[name] = new WhitenLayer(this, paramsDict, replicaID);
    } else if (type == "unpool") {
        _nameLayerMap[name] = &UnPoolLayer::make(this, paramsDict, replicaID);
    } else if (type == "cmunpool") {
        _nameLayerMap[name] = &CrossMapUnPoolLayer::make(this, paramsDict, replicaID);
    } else if (type == "convdec") {
        _nameLayerMap[name] = new ConvDecoderLayer(this, paramsDict, replicaID);
    } else {
...
//    NVMatrix::syncStream();
    ...
    srand(time(0));
    //cudaDeviceSetLimit(cudaLimitPrintfFifoSize, 20971520); // for debug only (yikes)
}

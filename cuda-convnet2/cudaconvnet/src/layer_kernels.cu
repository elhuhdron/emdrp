...
/*
 * error for "multiple independent attributes", see Bishop 6.8 eqn 6.145
 * E = -( target*log(y_t) + (1 - target)*log(1-y_t) )
 * probs:           (numOut, numCases)
 * labels:          (numOut, numCases)
 * logProbs:        (numOut, numCases)
 * logError:        (numOut, numCases)   (*out)
 * correctProbs:    (numOut, numCases)   (*out)
 */
__global__ void kIndepCrossEntCost(float* probs, float* labels, float* logProbs, float* logError, float* correctProbs, 
        const int numCases, const int numOut) {
    const int tx = blockIdx.x * LOGREG_GRAD_THREADS_X + threadIdx.x;
    const int ty = blockIdx.y * LOGREG_GRAD_THREADS_Y + threadIdx.y;
    const int tidx = ty * numCases + tx;
    ...
    if (ty < numOut && tx < numCases) {
        const float logProb = logProbs[tidx];
        //assert( logProb <= 0 ); // comment me after debugging, asserts in kernels are super slow
        const bool target = (labels[tidx] == 1.0); // for non-binary targets, use sum squared cost

        if( target ) {
            logError[tidx] = logProb;
        } else {
            if( logProb == 0 ) 
                // logProb of zero should usually not happen in a good training regime, but handle it anyways
                // normalized: log(single(realmin('single'))) is -87.337
                // denormalized: log(single(2^-149)) = -103.2789
                logError[tidx] = -104;
            // see "Accurately Computing log(1-exp(-|a|))" by Machler (2012), eqn (7)
            else if( logProb >= -0.693147181f ) // log(2)
                logError[tidx] = logf( -expm1f(logProb) );
            else
                logError[tidx] = log1pf( -expf(logProb) );
        }
        
        // no winner take all, each output is either correct or incorrect, use fixed threshold
        if( (target && probs[tidx] > 0.5) || (!target && probs[tidx] < 0.5) ) correctProbs[tidx] = 1;
        else correctProbs[tidx] = 0;
    }        
}

void computeIndepCrossEntCost(NVMatrix& labels, NVMatrix& probs, NVMatrix& logProbs, NVMatrix& logError_out, 
        NVMatrix& correctProbs_out) {
    int numCases = probs.getLeadingDim(); 
    int numOut = probs.getFollowingDim(); 
    int axisOut = probs.isTrans();
    assert(labels.getNumElements() == numCases*numOut);
    // see comment in bpropActs for IndepCrossEntCostLayer
    //assert(!labels.isTrans()); assert(!probs.isTrans()); assert(!logProbs.isTrans());
    assert(labels.isContiguous()); assert(probs.isContiguous()); assert(logProbs.isContiguous());
    
    NVMatrix logError, correctProbs; // log probs calculated for all targets in non-exclusive case, Bishop 6.145
    logError.resize(probs); correctProbs.resize(probs);
    
    dim3 threads(LOGREG_GRAD_THREADS_X, LOGREG_GRAD_THREADS_Y);
    dim3 blocks(DIVUP(numCases, LOGREG_GRAD_THREADS_X), DIVUP(numOut, LOGREG_GRAD_THREADS_Y));
    cudaStream_t stream = NVMatrix::getDefaultStream();
    cudaFuncSetCacheConfig(kIndepCrossEntCost, cudaFuncCachePreferL1);
    kIndepCrossEntCost<<<blocks, threads, 0, stream>>>(probs.getDevData(), labels.getDevData(), logProbs.getDevData(), 
        logError.getDevData(), correctProbs.getDevData(), numCases, numOut);
    getLastCudaError("computeIndepCrossEntCost: Kernel execution failed");
    
    // no winner take all, each output is either correct or incorrect, get fraction correct out of number of outputs
    correctProbs.sum(axisOut,correctProbs_out);
    correctProbs_out.apply(NVMatrixOps::MultByScalar(1.0/numOut));
    logError.sum(axisOut, logError_out); // sum log probs over outputs
}

// logreg with or without softmax (Bishop 6.8 and 6.9) has the same gradient calculation.
template <bool add>
__global__ void kIndepCrossEntGrad(float* y_l, float* labels, float* dE_dx_l, const int numCases,
        const int numOut, const float gradCoeff) {
    const int tx = blockIdx.x * LOGREG_GRAD_THREADS_X + threadIdx.x;
    const int ty = blockIdx.y * LOGREG_GRAD_THREADS_Y + threadIdx.y;
    const int tidx = ty * numCases + tx;
    ...
    if (ty < numOut && tx < numCases) {
        // only subtle difference with kLogregSoftmaxGrad, label is target, not encoded target.
        // added separate functions to make this easier to plugin to Alex's code base.
        // do not need a separate kernel for this, but left for future flexibility.
        float v = gradCoeff * (labels[tidx] - y_l[tidx]);
        if (add) {
            dE_dx_l[tidx] += v;
        } else {
            dE_dx_l[tidx] = v;
...
void computeIndepCrossEntGrad(NVMatrix& labels, NVMatrix& probs, NVMatrix& target, bool add, float coeff) {
    int numCases = probs.getLeadingDim(); 
    int numOut = probs.getFollowingDim(); 
    assert(labels.getNumElements() == numCases*numOut);
    // see comment in bpropActs for IndepCrossEntCostLayer
    //assert(!probs.isTrans()); assert(!labels.isTrans()); assert(!target.isTrans());
    assert(probs.isContiguous()); assert(labels.isContiguous()); assert(target.isContiguous());
    ...
    if (!add) {
        target.resize(probs);
        kIndepCrossEntGrad<false><<<blocks, threads, 0, stream>>>(probs.getDevData(), labels.getDevData(), 
            target.getDevData(), numCases, numOut, coeff);
    } else {
        kIndepCrossEntGrad<true><<<blocks, threads, 0, stream>>>(probs.getDevData(), labels.getDevData(), 
            target.getDevData(), numCases, numOut, coeff);
    }
    getLastCudaError("computeIndepCrossEntGrad: Kernel execution failed");
}

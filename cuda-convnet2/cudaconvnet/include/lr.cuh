...
public:
    ...
    static ParameterSchedule& make(PyObject* schedDict, int numEpochs);
};
...
class DiscreteExpTauParameterSchedule : public DiscreteExpParameterSchedule {
public:
    // Alex's definition of numSteps is not intuitive, the floor is a step, I say each change is a step, so +1
    DiscreteExpTauParameterSchedule(double baseRate, double tau, int numEpochs, int stepsPerEpoch)
        : DiscreteExpParameterSchedule( baseRate, 1.0 / std::exp(-numEpochs / tau), 
            (stepsPerEpoch > 0 ? stepsPerEpoch*numEpochs : numEpochs)+1 ) { }
};

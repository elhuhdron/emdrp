...
protected:
    ...
    virtual NVMatrix& getWeightMatrix(PASS_TYPE passType, int inpIdx);
...
protected:
    ...
    bool _flipWeights;
    NVMatrix _weightsFlipped, _weightsGradFlipped, _weightsGradUnflipped;
    ...
    virtual NVMatrix& getWeightMatrix(PASS_TYPE passType, int inpIdx);
    virtual NVMatrix& getWeightGrad(PASS_TYPE passType, int inpIdx);
    virtual void setWeightGrad(PASS_TYPE passType, int inpIdx, float scaleTargets);
...
public:
    ...
    virtual void bpropActs(NVMatrix& v, int replicaIdx, int inpIdx, float scaleTargets, PASS_TYPE passType);
    void truncBwdActs();
    DropoutLayer(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID, bool trans);
...
class UnPoolLayer : public Layer, public TwoDLayerInterface {
protected:
    int _sizeX, _start, _stride, _outputsX;
    std::string _pool;
    NVMatrix _tmppool;
    float _scale;
public:
    UnPoolLayer(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID, bool trans);
    
    static UnPoolLayer& make(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID);
}; 

class AvgUnPoolLayer : public UnPoolLayer {
protected:
    void fpropActs(int inpIdx, float scaleTargets, PASS_TYPE passType, int passIdx);
    void bpropActs(NVMatrix& v, int replicaIdx, int inpIdx, float scaleTargets, PASS_TYPE passType);
public:
    AvgUnPoolLayer(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID);
}; 

class MaxUnPoolLayer : public UnPoolLayer {
protected:
    bool _abs;
    void fpropActs(int inpIdx, float scaleTargets, PASS_TYPE passType, int passIdx);
    void bpropActs(NVMatrix& v, int replicaIdx, int inpIdx, float scaleTargets, PASS_TYPE passType);
public:
    MaxUnPoolLayer(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID, bool abs);
};

class CrossMapUnPoolLayer : public Layer, public TwoDLayerInterface {
protected:
    int _size, _start, _stride, _outputs;
    std::string _pool;
    NVMatrix _tmppool;
public:
    CrossMapUnPoolLayer(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID, bool trans);

    static CrossMapUnPoolLayer& make(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID);
};

class CrossMapMaxUnPoolLayer : public CrossMapUnPoolLayer {
protected:
    void fpropActs(int inpIdx, float scaleTargets, PASS_TYPE passType, int passIdx);
    void bpropActs(NVMatrix& v, int replicaIdx, int inpIdx, float scaleTargets, PASS_TYPE passType);
public:
    CrossMapMaxUnPoolLayer(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID);
};

class SumLayer : public Layer {
protected:
    int _stride, _noutputs;
    NVMatrix _cursum;
    void fpropActs(int inpIdx, float scaleTargets, PASS_TYPE passType, int passIdx);
    void bpropActs(NVMatrix& v, int replicaIdx, int inpIdx, float scaleTargets, PASS_TYPE passType);
public:
...
class WhitenLayer : public Layer, public TwoDLayerInterface {
protected:
    Matrix* _hwhiten;
    NVMatrix _whiten, _actGradsTmp;
    float _wmean, _wstd;
public:
    void fpropActs(int inpIdx, float scaleTargets, PASS_TYPE passType, int passIdx);
    void bpropActs(NVMatrix& v, int replicaIdx, int inpIdx, float scaleTargets, PASS_TYPE passType);
    ...
    WhitenLayer(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID);
    ~WhitenLayer();
};

class IndepCrossEntCostLayer : public CostLayer {
protected:
    NVMatrix _logProbs, _logError, _correctProbs;
    void fpropActs(int inpIdx, float scaleTargets, PASS_TYPE passType, int passIdx);
    void bpropActs(NVMatrix& v, int replicaIdx, int inpIdx, float scaleTargets, PASS_TYPE passType);
public:
    IndepCrossEntCostLayer(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID);
    //~IndepCrossEntCostLayer();
    //void copyToGPU();
    //void copyToCPU();
};

class NormPCostLayer : public CostLayer {
protected:
    NVMatrix _tmp, _tmpGrad1, _tmpGrad2;
    int _p;
    float _divideInput, _alpha;
    void fpropActs(int inpIdx, float scaleTargets, PASS_TYPE passType, int passIdx);
    void bpropActs(NVMatrix& v, int replicaIdx, int inpIdx, float scaleTargets, PASS_TYPE passType);
public:
    NormPCostLayer(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID);
};

class ConvDecoderLayer : public ConvLayer {
protected:
    int _nPadFilters;
    bool _sumFeatures;
    NVMatrix _tmpTarget, _tmpWeights, _tmpVbw, _tmpVbp, _tmpGrad, _tmpGradP;
...
public:
    ConvDecoderLayer(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID);
    //virtual ~ConvDecoderLayer();
}; 

...
#        self.dic['conserveMem'] = mcp.convnet.op.get_value('conserve_mem') if mcp.convnet is not None else 0
    ...
    def parse(self, name, mcp, prev_layers, model=None):
        ...
        if model: self.init_load_path = model.init_load_path
...
#        dic['wballNormed'] = [wball * nweights for wball,nweights in zip(dic['wball'], dic['weightsPerFilter'])]
    ...
    def call_init_func(self, param_name, shapes, input_idx=-1):
        ...
        # have load path defined from command line for loading weights dumped from another trained net
        try:
            i = params.index('init_load_path')
            params[i] = self.init_load_path
        except:
            pass
        try:
    ...
    def parse(self, name, mcp, prev_layers, model):
        ...
        dic['num_epochs'] = model.op.get_value('num_epochs')    # for discrete learning sched with fixed tau
...
class ConvLayerParser(LocalLayerParser):
    ...
    def parse(self, name, mcp, prev_layers, model):
        ...
        dic['flipWeights'] = mcp.safe_get_bool(name, 'flipWeights', default=False)
...
class SumOfSquaresCostParser(CostParser):
    ...
    def parse(self, name, mcp, prev_layers, model):
        dic = CostParser.parse(self, name, mcp, prev_layers, model)
        dic['requiresParams'] = True
        dic['outputFilterFormatter'] = 'lambda self,costs: ", ".join("%g" % v for v in costs)'
    ...
    def add_params(self, mcp):
        ...
        dic, name = self.dic, self.dic['name']
        dic['divideInput'] = mcp.safe_get_float(name, 'divideInput', default=1.0)
        dic['alpha'] = mcp.safe_get_float(name, 'alpha', default=1.0)

class NormPCostParser(CostParser):
    def __init__(self):
        CostParser.__init__(self, num_inputs=1)
    ...
    def parse(self, name, mcp, prev_layers, model):
        ...
        dic['requiresParams'] = True
        dic['outputFilterFormatter'] = 'lambda self,costs: ", ".join("%g" % v for v in costs)'
        dic['p'] = mcp.safe_get_int(name, 'p')
        self.verify_num_range(dic['p'], 'p', 1, None)
        print "Initialized %d-norm cost '%s' on GPUs %s" % (dic['p'], name, dic['gpus'])
        return dic
...
# logistic sigmoid is applied in this layer and the outputs are the probs
class IndepCrossEntCostParser(CostParser):
    def __init__(self):
        CostParser.__init__(self, num_inputs=2)
    ...
    def add_params(self, mcp):
        ...
        # uncommented mcp.convnet = model above so these links can be reset... xxx - better idea?
        mcp.convnet.train_data_provider.batch_meta['prior_train_count'] = dic['prior_train_count']
        mcp.convnet.train_data_provider.batch_meta['prior_total_count'] = dic['prior_total_count']
        
    def parse(self, name, mcp, prev_layers, model):
        dic = CostParser.parse(self, name, mcp, prev_layers, model)
        dic['requiresParams'] = True

        # number of independent outputs, noutputs in parseEMdata
        dic['outputs'] = dic['numInputs'][1]
        
        # allocate saving counts for training labels here so they can be saved with convnet checkpoints
        dic['prior_train_count'] = n.zeros((dic['outputs'],),dtype=n.int64)
        dic['prior_total_count'] = n.zeros((1,),dtype=n.int64)
        model.train_data_provider.batch_meta['prior_train_count'] = dic['prior_train_count']
        model.train_data_provider.batch_meta['prior_total_count'] = dic['prior_total_count']
        
        # xxx - incase this layer needs to know what the outputs are (parseEMdata usually handles all this)
        #dic['sizeX'] = mcp.safe_get_int(name, 'sizeX')  # this is image_out_size in parseEMdata
        #self.verify_divisible(dic['outputs'], dic['sizeX'], 'total outputs', 'output size')
        #dic['imgPixels'] = dic['sizeX']*dic['sizeX']    # this is pixels_per_out_image in parseEMdata
        #dic['nTypes'] = dic['outputs'] / dic['imgPixels']   # this is nIndepLabels in parseEMdata

        if dic['outputs'] != model.train_data_provider.get_num_classes():
            raise LayerParsingError("Layer '%s': indep cost input '%s' must produce %d outputs, because that is the number of classes in the dataset" \
                                    % (name, dic['inputs'][1], model.train_data_provider.get_num_classes()))
        # the labels are independent and encoded as binary targets (0.0 or 1.0).
        # could potentially have labels as target output probabilities, but currently no need for this.
        if dic['numInputs'][0] != dic['outputs']:
            raise LayerParsingError("Layer '%s': both inputs must produce the same number of outputs" % (name))
        #if dic['applyLogistic'] and 'neuron' in dic['inputLayers'][1] and dic['inputLayers'][1]['neuron'] not in set(('ident', '')):
        if 'neuron' not in dic['inputLayers'][1] or dic['inputLayers'][1]['neuron'] != 'ident':
            raise LayerParsingError("Layer '%s': input '%s' is not ident, logistic applied by indep cost layer." % (dic['name'], dic['inputs'][1]))

        dic['outputFilterFormatter'] = 'lambda self,costs: ", ".join("%g" % v for v in costs)'
        
        #print "Initialized independent cross-entropy cost '%s' producing %dx%d outputs for %d types on GPUs %s" % (name, 
        #    dic['sizeX'], dic['sizeX'], dic['nTypes'], dic['gpus'])
        print "Initialized independent cross-entropy cost '%s' on GPUs %s" % (name, dic['gpus'])
        return dic
...
class SumLayerParser(LayerWithInputParser):
    ...
    def add_params(self, mcp):
        LayerWithInputParser.add_params(self, mcp)
        dic, name = self.dic, self.dic['name']
        #dic['coeff'] = mcp.safe_get_float(name, 'coeff', default=1.0)

class UnPoolLayerParser(LayerWithInputParser):
    def __init__(self):
        LayerWithInputParser.__init__(self, num_inputs=1)
    ...
    def add_params(self, mcp):
        ...
        dic, name = self.dic, self.dic['name']
        #dic['enable'] = mcp.safe_get_bool(name, 'enable', default=True)
        #dic['scale'] = mcp.safe_get_float(name, 'scale', default=1.0)
    ...
    def parse(self, name, mcp, prev_layers, model):
        ...
        dic['stride'] = mcp.safe_get_int(name, 'stride')
        dic['outputsX'] = mcp.safe_get_int(name, 'outputsX', default=0)
        #dic['outputsX'] = 0    # xxx - not sure what the point of having this non-zero would be
        dic['pool'] = mcp.safe_get(name, 'pool')
        #dic['requiresParams'] = True
        ...
        #dic['outputsPix'] = dic['outputsX'] * dic['outputsX']
        #dic['scale'] = float(dic['imgPixels']) / dic['outputsPix'];
        
        #print "Initialized %s-pooling layer '%s' on GPUs %s, producing %dx%d %d-channel output" % (dic['pool'], name, dic['gpus'], dic['outputsX'], dic['outputsX'], dic['channels'])
        print "Initialized %s-unpooling layer '%s' on GPUs %s, producing %dx%d sparse for %dx%d %d-channel output" % (dic['pool'], name, dic['gpus'], dic['outputsX'], dic['outputsX'], dic['imgSize'], dic['imgSize'], dic['channels'])
        return dic

class CrossMapUnPoolLayerParser(LayerWithInputParser):
    def __init__(self):
        LayerWithInputParser.__init__(self, num_inputs=1)
    ...
    def parse(self, name, mcp, prev_layers, model):
        ...
        covered_chans = dic['start'] + (dic['outputChannels'] - 1) * dic['stride'] + dic['size']
        if covered_chans < dic['channels']:
            raise LayerParsingError("Layer '%s': cross-map unpooling with start=%d, stride=%d, size=%d, outputs=%d covers only %d of %d input channels." % \
                                    (name, dic['start'], dic['stride'], dic['size'], dic['outputChannels'], covered_chans, dic['channels']))
        
        #print "Initialized cross-map %s-pooling layer '%s' on GPUs %s, producing %dx%d %d-channel output" % (dic['pool'], name, dic['gpus'], dic['imgSize'], dic['imgSize'], dic['outputChannels'])
        print "Initialized cross-map %s-pooling layer '%s' on GPUs %s, producing %d-channel downsample to %d-channel %dx%d output" % (dic['pool'], name, dic['gpus'], dic['outputChannels'], dic['channels'], dic['imgSize'], dic['imgSize'])
        return dic


class ConvDecoderLayerParser(ConvLayerParser):
    def __init__(self):
        ConvLayerParser.__init__(self)
        
    def add_params(self, mcp):
        ConvLayerParser.add_params(self, mcp)
        dic, name = self.dic, self.dic['name']
        
        # xxx - from _imgActs "// TODO: insisting on 32 filters due to bug in calling code below. fix that."
        #   this was fixed in a113facf2c2b and incorporated here but min of 32 is still faster.... so there's that
        #self.dic['nPadFilters'] = 16
        self.dic['nPadFilters'] = 32

        dic['nPadFilters'] = max([self.dic['nPadFilters'], max(dic['filterChannels'])])
    ...
    def parse(self, name, mcp, prev_layers, model):
        ...
        dic['sharedBiases'] = mcp.safe_get_bool(name, 'sharedBiases', default=True)
        dic['flipWeights'] = mcp.safe_get_bool(name, 'flipWeights', default=False)
        dic['sumFeatures'] = mcp.safe_get_bool(name, 'sumFeatures', default=False)
        
        # a set of filters is only applied each to one channel, groups does not work for this b/c of reqs
        for i in range(len(dic['channels'])):
            self.verify_divisible(dic['filters'], dic['channels'][i], 'filters', 'channels', input_idx=i)
        dic['filterChannels'] = [dic['filters']/chan for chan in dic['channels']]
        dic['weightsPerFilter'] = [fc * (fz**2) for fc, fz in zip(dic['filterChannels'], dic['filterSize'])]
        
        if dic['sumFeatures']: 
            dic['outputs'] = dic['modules'] * dic['filterChannels'][0]
            num_biases = dic['filterChannels'][0] if dic['sharedBiases'] else dic['modules']*dic['filterChannels'][0]
        else:
            num_biases = dic['filters'] if dic['sharedBiases'] else dic['modules']*dic['filters']

        #eltmult = lambda list1, list2: [l1 * l2 for l1,l2 in zip(list1, list2)]
        #self.make_weights(dic['initW'], eltmult(dic['filterPixels'], dic['filterChannels']), [dic['filters']] * len(dic['inputs']), order='C')
        self.make_weights(dic['initW'], dic['filterPixels'], [dic['filters']] * len(dic['inputs']), order='C')
        self.make_biases(num_biases, 1, order='C')

        #if dic['flipWeights'] and not all([bool(src) for src in dic['weightSource']]):
        #    raise LayerParsingError("Layer '%s': convolutional decoder only use flipWeights when tying (sharing) weights" % (dic['name'],))
        #if not all([chan == dic['filters'] for chan in dic['channels']]):
        #    raise LayerParsingError("Layer '%s': convolutional decoder needs %d channels != %d filters" % (dic['name'],dic['channels'][0],dic['filters']))

        print "Initialized convolutional decoder layer '%s' on GPUs %s, %s sum producing %dx%d %d-channel output, %d filter(s) per channel" % (name, dic['gpus'], 'with' if dic['sumFeatures'] else 'without', dic['modulesX'], dic['modulesX'], dic['outputs'] / dic['modules'], dic['filterChannels'][0])
        return dic    

class WhitenLayerParser(LayerWithInputParser):
    def __init__(self):
        LayerWithInputParser.__init__(self, num_inputs=1)
    ...
    def add_params(self, mcp):
        LayerWithInputParser.add_params(self, mcp)
        dic, name = self.dic, self.dic['name']
        dic['wmean'] = mcp.safe_get_float(name, 'mean', default=0.0)
        dic['wstd'] = mcp.safe_get_float(name, 'std', default=1.0)
    ...
    def parse(self, name, mcp, prev_layers, model=None):
        ...
        dic['channels'] = mcp.safe_get_int(name, 'channels')
        dic['whiten_load'] = mcp.safe_get(name, 'whiten_load', default="")
        if not os.path.isfile(dic['whiten_load']):
            dic['whiten_load'] = os.path.join(model.op.get_value('init_load_path'),
                os.path.basename(dic['whiten_load']))
        dic['wdtype'] = mcp.safe_get(name, 'wdtype', default="double")
        ...
        self.verify_str_in(dic['wdtype'], 'wdtype', ['single','double'])
        dtype = eval('n.' + dic['wdtype'])
        dic['whiten'] = n.fromfile(dic['whiten_load'],
            dtype=dtype).reshape((dic['imgPixels'],dic['imgPixels']))
        fabs = n.fabs(dic['whiten'])
        assert( (fabs >= n.finfo(n.single).tiny).all() and (fabs <= n.finfo(n.single).max).all() ) # make sure single ok
        dic['whiten'] = dic['whiten'].astype(n.single)
        
        print "Initialized whitening layer '%s', using '%s' dtype %s producing %dx%d %d-channel output" % (name, 
            dic['whiten_load'], dic['wdtype'], dic['imgSize'], dic['imgSize'], dic['channels'])
...
layer_parsers = {'data' :           lambda : DataLayerParser(),
                 ...
                 'cost.ice':        lambda : IndepCrossEntCostParser(),
                 'cost.normp':      lambda : NormPCostParser(),
                 'sum':             lambda : SumLayerParser(),
                 'unpool':          lambda : UnPoolLayerParser(),
                 'cmunpool':        lambda : CrossMapUnPoolLayerParser(),
                 'convdec':         lambda : ConvDecoderLayerParser(),
                 'whiten':          lambda : WhitenLayerParser()}
...
lrs_parsers = sorted([ParamParser('const[fbase]'),
                      ...
                      ParamParser('texp[fbase;ftau;istepsPerEpoch]')])

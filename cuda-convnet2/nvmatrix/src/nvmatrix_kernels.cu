...
__global__ void kFlipContiguous(const float* src, float* tgt, const uint width, const uint height, const uint side) {
    const int idx = blockIdx.x * blockDim.x + threadIdx.x;
    const int numThreads = blockDim.x * gridDim.x;
    for (uint i = idx; i < width * height; i += numThreads) {
        const uint y = i / width;
        const uint x = i % width;
        const uint tgtsx = side - (x % side) - 1;
        const uint tgtsy = side - (x / side) - 1;
        const uint tgtx = tgtsy * side + tgtsx;
        tgt[y * width + tgtx] = src[i];
    }
}

__global__ void kFlipNonContiguous(const float* src, float* tgt, const uint width, const uint height, const uint side) {
    const int idx = blockIdx.x * blockDim.x + threadIdx.x;
    const int numThreads = blockDim.x * gridDim.x;
    for (uint i = idx; i < width * height; i += numThreads) {
        const uint y = i / width;
        const uint x = i % width;
        const uint tgtsx = side - (y % side) - 1;
        const uint tgtsy = side - (y / side) - 1;
        const uint tgty = tgtsy * side + tgtsx;
        tgt[tgty * width + x] = src[i];
        //tgt[i] = src[i];
    }
}

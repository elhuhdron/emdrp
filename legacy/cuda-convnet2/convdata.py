...
class CIFARDataProvider(LabeledDataProvider):
    def __init__(self, data_dir, batch_range=None, init_epoch=1, init_batchnum=None, dp_params=None, test=False):
        ...
        self.num_views = 9
        #self.scalar_mean = dp_params['scalar_mean'] 
        self.scalar_mean = dp_params['scalar_mean'] if dp_params['scalar_mean'] > 0 else 0
        self.data_mult = self.num_views if self.multiview else 1
        self.data_dic = []
        for i in batch_range:
            ...
            #for j in range(self.data_dic[-1]['data'].shape[1]):
            #    tmp = self.data_dic[-1]['data'][:,j].reshape(3,self.img_size,self.img_size)
            #    pl.figure(1);
            #    pl.imshow(tmp.transpose((1,2,0)),interpolation='nearest');
            #    pl.show()
    ...
    def get_next_batch(self):
        ...
        if self.border_size > 0: 
            ...
            cropped[:,:] = self.data_dic[bidx]['data']

        cropped -= self.data_mean
        self.batches_generated += 1
        return epoch, batchnum, [cropped, self.data_dic[bidx]['labels']]
        #return epoch, batchnum, [cropped - self.data_mean, cropped]
        #return epoch, batchnum, [cropped, cropped/self.num_colors]
        
    def get_data_dims(self, idx=0):
        return self.inner_size**2 * self.num_colors if idx == 0 else 1
        #return self.inner_size**2 * self.num_colors

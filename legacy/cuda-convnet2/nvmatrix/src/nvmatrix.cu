    ...
    if (numRows != _numRows || numCols != _numCols) {
        ...
        if (_numElements != numRows * numCols) {
            ...
            if (numRows * numCols > 0) { // allocate new memory
                ...
                // make sure this isn't because of a 32-bit pointer
...
void NVMatrix::flipSquare(int axis, NVMatrix& target) {
    flipSquare(axis, target, getDefaultStream());
}

void NVMatrix::flipSquare(int axis, NVMatrix& target, cudaStream_t stream) {
    assert(isContiguous() && target.isContiguous() && axis < 2 );
    int numAxis = (axis ?  _numCols : _numRows), squareSide = (int) sqrt((float)numAxis);
    assert( numAxis % squareSide == 0 );
    target.setTrans(_isTrans); target.resize(_numRows, _numCols);

    int width = getLeadingDim(), height = getFollowingDim();    
    if( isTrans() == axis ) {
        kFlipNonContiguous<<<NUM_FLIP_BLOCKS,NUM_FLIP_THREADS_PER_BLOCK, 0, stream>>>(getDevData(), target.getDevData(), getLeadingDim(), getFollowingDim(), squareSide);
    } else {
        kFlipContiguous<<<NUM_FLIP_BLOCKS,NUM_FLIP_THREADS_PER_BLOCK, 0, stream>>>(getDevData(), target.getDevData(), getLeadingDim(), getFollowingDim(), squareSide);
    }
    getLastCudaError("flipSquare: Kernel execution failed");
}

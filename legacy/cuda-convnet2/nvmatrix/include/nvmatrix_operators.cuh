...
public:
    ...
    public:
        __device__ inline float operator()(const float a) const {
            // log of logistic(a) equals log (1 / (1 + exp(-a)) equals -log( 1 + exp(-a) )
            // very large negative input should not usually happen, but handle incase (usually bad training regime)
            // log(realmax('single')) = 88.72284
            return a < -88 ? a : -log1pf(expf(-a));
        }
    };
    ...
    private:
        const float p;
    public:
        PowAbs(const float _p) : p(_p) {
        }
        __device__ inline float operator()(const float a) const {
            return __powf(fabsf(a), p);
        }
    };

    class NotEqualToScalar {
    private:
        const float scalar;
    public:
        NotEqualToScalar(const float _scalar) : scalar(_scalar) {
        }
        __device__ inline float operator()(const float a) const {
            return a != scalar;
        }

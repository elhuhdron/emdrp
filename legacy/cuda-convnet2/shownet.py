...
try:
    ...
    pass # pwatkins - problems when trying to queue jobs with just feature writer, ignore this import error
...
#                num_filters *= layer['modules']
                ...
                filters = filters.reshape(channels, filters.shape[0]/channels, filters.shape[1])
        elif layer['type'] in ('convdec'): # conv decoder layer
            num_filters = layer['channels'][self.input_idx]
            channels = layer['filterChannels'][self.input_idx]
            filters = filters.reshape(filters.shape[0], filters.shape[1]/channels, channels).transpose(2,0,1)
        ...
        # pwatkins - export raw in F-order
        filter_size = int(sqrt(filters.shape[1])); fn = 'filters_%s_chns%d_filters%d_size%d_float32.raw' \
            % (self.show_filters,filters.shape[0],filters.shape[2],filter_size)
        out_raw = open(os.path.join('outfigs', fn), 'wb'); filters.reshape((filters.shape[0],filter_size,filter_size,
            filters.shape[2])).transpose(0,3,2,1).tofile(out_raw); out_raw.close()
...
#        print self.show_cost
    ...
    def get_options_parser(cls):
        ...
        for option in list(op.options):
            if option not in ('gpu', 'load_file', 'inner_size', 'train_batch_range', 'test_batch_range', 'multiview_test', 'data_path', 'pca_noise', 'scalar_mean', 'em_feature_path'):

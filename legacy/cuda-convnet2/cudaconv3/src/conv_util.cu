...
/*
 * hacked version of kLocalMaxUndo where grads (previously maxGrads) is the same size as images
 * Block size B_YxB_X
 * blockIdx.x determines pixel.x, image idx in batches of B_X*imgsPerThread
 * blockIdx.y determines pixel.y, filter idx in batches of B_Y*filtersPerThread
 * 
 ...
 * 
 * imgs:        (numFilters, imgPixels, numImages)
 * grads:       (numFilters, imgPixels, numImages)
 * maxActs:    (numFilters, numOutputs, numImages)
 * target:      (numFilters, imgPixels, numImages)
 * 
 * numImages must be divisible by B_X*imgsPerThread
 ...
 */
template<int B_Y, int B_X, int imgsPerThread, int filtersPerThread, bool add, bool checkCaseBounds>
__global__ void kLocalMaxUnpool(float* imgs, float* grads, float* maxActs, float* target, const int imgSize, const int numFilters,
                              const int numImages, const int subsX, const int startX, const int strideX, const int outputsX,
                              const float scaleTargets, const float scaleOutputs) {
    __shared__ float shImgs[B_Y*filtersPerThread][B_X*imgsPerThread];
    const int numImgBlocks = DIVUP(numImages,B_X*imgsPerThread);
    ...
    //maxGrads += ((blockFilterIdx + threadIdx.y) * numOutputs) * numImages
    //        + imgIdx;
    grads += ((blockFilterIdx + threadIdx.y) * imgPixels + blockPx) * numImages + imgIdx;
    maxActs += ((blockFilterIdx + threadIdx.y) * numOutputs) * numImages
            + imgIdx;
                ...
                for (int i = 0; i < imgsPerThread; i++) {
                    if (!checkCaseBounds || imgIdx + i * B_X < numImages) {
                        ...
                        for (int f = 0; f < filtersPerThread; f++) {
                            const float ma = maxActs[(f * B_Y * numOutputs + outputIdx) * numImages + i * B_X];
                            //const float mg = maxGrads[(f * B_Y * numOutputs + outputIdx) * numImages + i * B_X];
                            const float mg = grads[f * B_Y * imgPixels * numImages + i * B_X];
                            const float img = shImgs[threadIdx.y + B_Y * f][threadIdx.x + B_X * i];
...
/*
 * hacked version of convLocalMaxUndo where grads (previously maxGrads) is the same size as images
 * imgs:        (numFilters, imgPixels, numImages)
 * grads:       (numFilters, imgPixels, numImages)
 * maxActs:     (numFilters, numOutputs, numImages)
 * target:      (numFilters, imgPixels, numImages)
...
void convLocalMaxUnpool(NVMatrix& images, NVMatrix& grads, NVMatrix& maxActs, NVMatrix& target,
    ...
    int numImages = images.getNumCols();
    //int numFilters = maxGrads.getNumRows() / outputs;
    int numFilters = maxActs.getNumRows() / outputs;
    int imgPixels = images.getNumRows() / numFilters;
    assert(images.getNumRows() == numFilters * imgPixels);
    int imgSize = int(sqrt(imgPixels));
    
    assert(imgSize * imgSize == imgPixels);
    assert(maxActs.getNumRows() == numFilters * outputs);
    assert(maxActs.getNumCols() == numImages);
    assert(!images.isTrans());
    assert(!target.isTrans());
    //assert(!maxGrads.isTrans());
    assert(!grads.isTrans());
    assert(!maxActs.isTrans());
    assert(images.isContiguous());
    //assert(maxGrads.isContiguous());
    assert(grads.isContiguous());
    assert(maxActs.isContiguous());
    //assert(maxGrads.isSameDims(maxActs));
    assert(grads.isSameDims(images));
    assert(numFilters % 16 == 0);
//    assert(numImages % 128 == 0);
    ...
    if (imgsPerThread == 4) {
        if  (checkCaseBounds) {
            if (scaleTargets == 0 && scaleOutput == 1) {
                kLocalMaxUnpool<4, 32, 4, 2, false, true><<<blocks, threads, 0, stream>>>(images.getDevData(), grads.getDevData(), maxActs.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            } else {
                kLocalMaxUnpool<4, 32, 4, 2, true, true><<<blocks, threads, 0, stream>>>(images.getDevData(), grads.getDevData(), maxActs.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            }
        } else {
            if (scaleTargets == 0 && scaleOutput == 1) {
                kLocalMaxUnpool<4, 32, 4, 2, false, false><<<blocks, threads, 0, stream>>>(images.getDevData(), grads.getDevData(), maxActs.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            } else {
                kLocalMaxUnpool<4, 32, 4, 2, true, false><<<blocks, threads, 0, stream>>>(images.getDevData(), grads.getDevData(), maxActs.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            }
        }
    } else if (imgsPerThread == 2) {
        if  (checkCaseBounds) {
            if (scaleTargets == 0 && scaleOutput == 1) {
                kLocalMaxUnpool<4, 32, 2, 2, false, true><<<blocks, threads, 0, stream>>>(images.getDevData(), grads.getDevData(), maxActs.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            } else {
                kLocalMaxUnpool<4, 32, 2, 2, true, true><<<blocks, threads, 0, stream>>>(images.getDevData(), grads.getDevData(), maxActs.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            }
        } else {
            if (scaleTargets == 0 && scaleOutput == 1) {
                kLocalMaxUnpool<4, 32, 2, 2, false, false><<<blocks, threads, 0, stream>>>(images.getDevData(), grads.getDevData(), maxActs.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            } else {
                kLocalMaxUnpool<4, 32, 2, 2, true, false><<<blocks, threads, 0, stream>>>(images.getDevData(), grads.getDevData(), maxActs.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            }
        }
    } else {
        if  (checkCaseBounds) {
            if (scaleTargets == 0 && scaleOutput == 1) {
                kLocalMaxUnpool<4, 32, 1, 2, false, true><<<blocks, threads, 0, stream>>>(images.getDevData(), grads.getDevData(), maxActs.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            } else {
                kLocalMaxUnpool<4, 32, 1, 2, true, true><<<blocks, threads, 0, stream>>>(images.getDevData(), grads.getDevData(), maxActs.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            }
        } else {
            if (scaleTargets == 0 && scaleOutput == 1) {
                kLocalMaxUnpool<4, 32, 1, 2, false, false><<<blocks, threads, 0, stream>>>(images.getDevData(), grads.getDevData(), maxActs.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            } else {
                kLocalMaxUnpool<4, 32, 1, 2, true, false><<<blocks, threads, 0, stream>>>(images.getDevData(), grads.getDevData(), maxActs.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            }
        }
    }

    getLastCudaError("convLocalMaxUnpool: kernel execution failed");
}

/*
 * hacked version of kCrossMapMaxPoolUndo where grads (previously maxGrads) is the same size as images
 * Block size B_YxB_X
 * blockIdx.x determines pixel.x, image idx in batches of B_X*imgsPerThread
 * blockIdx.y determines pixel.y, filter idx in batches of B_Y
 * 
 ...
 */
template<int B_Y, int B_X, int imgsPerThread, bool add, bool checkCaseBounds>
__global__ void kCrossMapMaxUnPool(float* imgs, float* grads, float* maxActs, float* target, const int imgSize, const int numFilters,
                                     const int numImages, const int startF, const int poolSize,
                                     const int numOutputs, const int stride, const float scaleTargets, const float scaleOutputs) {
    const int numImgBlocks = DIVUP(numImages,B_X*imgsPerThread);
//    const int numOutputs = DIVUP(numFilters, stride);
    ...
    imgs            += ((filterIdx) * imgPixels + pxIdx) * numImages + imgIdx;
    //maxGrads        += (/*(filterIdx) * imgPixels +*/ pxIdx) * numImages + imgIdx;
    grads           += ((filterIdx) * imgPixels + pxIdx) * numImages + imgIdx;
    maxActs         += (/*(filterIdx) * imgPixels +*/ pxIdx) * numImages + imgIdx;
    target          += ((filterIdx) * imgPixels + pxIdx) * numImages + imgIdx;
 ...
 * target:              (numFilters, imgPixels, numImages)
 */
void convCrossMapMaxUnPool(NVMatrix& images, NVMatrix& grads, NVMatrix& maxActs, NVMatrix& target,
                             const int imgSize, const int startF, const int poolSize,
                             const int stride, const float scaleTargets, const float scaleOutputs) {
    int numImages = images.getNumCols();
    int imgPixels = imgSize * imgSize;
    ...
    //assert(maxGrads.getNumRows() == numOutputs * imgPixels);
    //assert(maxGrads.getNumCols() == numImages);
    assert(maxActs.getNumRows() == numOutputs * imgPixels);
    assert(maxActs.getNumCols() == numImages);
    //assert(maxGrads.isSameDims(maxActs));
    assert(grads.isSameDims(images));
...
//    assert(numImages % 128 == 0);
    ...
    if (scaleTargets == 0) {
        ...
        if (!checkCaseBounds) {
            if (imgsPerThread == 4) {
                kCrossMapMaxUnPool<4, 32, 4, false, false><<<blocks, threads, 0, stream>>>(images.getDevData(), grads.getDevData(), maxActs.getDevData(), target.getDevData(),
                                                                                             imgSize, numFilters, numImages, startF, poolSize, numOutputs, stride,
                                                                                             scaleTargets, scaleOutputs);
            } else if (imgsPerThread == 2) {
                kCrossMapMaxUnPool<4, 32, 2, false, false><<<blocks, threads, 0, stream>>>(images.getDevData(), grads.getDevData(), maxActs.getDevData(), target.getDevData(),
                                                                                             imgSize, numFilters, numImages, startF, poolSize, numOutputs, stride,
                                                                                             scaleTargets, scaleOutputs);
            } else {
                kCrossMapMaxUnPool<4, 32, 1, false, false><<<blocks, threads, 0, stream>>>(images.getDevData(), grads.getDevData(), maxActs.getDevData(), target.getDevData(),
                                                                                             imgSize, numFilters, numImages, startF, poolSize, numOutputs, stride,
                                                                                             scaleTargets, scaleOutputs);
            }
        } else {
            kCrossMapMaxUnPool<4, 32, 1, false, true><<<blocks, threads, 0, stream>>>(images.getDevData(), grads.getDevData(), maxActs.getDevData(), target.getDevData(),
                                                                                         imgSize, numFilters, numImages, startF, poolSize, numOutputs, stride,
                                                                                         scaleTargets, scaleOutputs);
        }
    } else {
        ...
        if (!checkCaseBounds) {
            if (imgsPerThread == 4) {
                kCrossMapMaxUnPool<4, 32, 4, true, false><<<blocks, threads, 0, stream>>>(images.getDevData(), grads.getDevData(), maxActs.getDevData(), target.getDevData(),
                                                                                             imgSize, numFilters, numImages, startF, poolSize, numOutputs, stride,
                                                                                             scaleTargets, scaleOutputs);
            } else if (imgsPerThread == 2) {
                kCrossMapMaxUnPool<4, 32, 2, true, false><<<blocks, threads, 0, stream>>>(images.getDevData(), grads.getDevData(), maxActs.getDevData(), target.getDevData(),
                                                                                             imgSize, numFilters, numImages, startF, poolSize, numOutputs, stride,
                                                                                             scaleTargets, scaleOutputs);
            } else {
                kCrossMapMaxUnPool<4, 32, 1, true, false><<<blocks, threads, 0, stream>>>(images.getDevData(), grads.getDevData(), maxActs.getDevData(), target.getDevData(),
                                                                                             imgSize, numFilters, numImages, startF, poolSize, numOutputs, stride,
                                                                                             scaleTargets, scaleOutputs);
            }
        } else {
            kCrossMapMaxUnPool<4, 32, 1, true, true><<<blocks, threads, 0, stream>>>(images.getDevData(), grads.getDevData(), maxActs.getDevData(), target.getDevData(),
                                                                                         imgSize, numFilters, numImages, startF, poolSize, numOutputs, stride,
                                                                                         scaleTargets, scaleOutputs);
        }
    }

...
class CheckpointWriter(Thread):
    def __init__(self, path, dic, keep_saves=False):
        Thread.__init__(self)
        self.path = path
        self.dic = dic
        self.keep_saves = keep_saves
...
#            print self.options["save_file_override"].value, self.save_file
            ...
            self.model_state = {}
            self.save_file = self.options["save_file_override"].value if self.options["save_file_override"].value_given else os.path.join(self.options['save_path'].value, model_name + "_" + '_'.join(['%s_%s' % (char, self.options[opt].get_str_value()) for opt, char in filename_options]) + '_gpu' + '_'.join([str(x) for x in self.gpu]) + '_' + strftime('%Y-%m-%d_%H.%M.%S'))
...
#            print self.save_file
        dp_params['save_name'] = os.path.split(self.save_file)[1]
    ...
    def save_state(self):
        ...
        self.checkpoint_writer = CheckpointWriter(checkpoint_file_full_path, dic, self.keep_saves)
    ...
    def get_options_parser():
        ...
        op.add_option("test-one", "test_one", BooleanOptionParser, "Test on one batch at a time?", default=0)
        ...
        op.add_option("keep-saves", "keep_saves", BooleanOptionParser, "Do not delete previously saved checkpoints", 
            default=0)
        return op
